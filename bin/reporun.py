...
def apply_config(config, basedir):
    ...
    for file in range(len(config['files'])):
        results = {}
        print config['files'][file]['source']
        print config['files'][file]['destination']
        try:
            if os.path.isdir('%s/%s' % (basedir, config['files'][file]['source'])):
                mkdir_p(config['files'][file]['destination'])
                shutil.copytree('%s/%s' % (basedir,config['files'][file]['source']),
                                '%s/%s' % (config['files'][file]['destination'], config['files'][file]['source']))
            else:
